...
echo
opt1=$(gum choose --height=15 "monitor" "waybar " "blur" "corners" "animations" "window gaps" "backgrounds" "eye protection" "manual settings" "default apps" "exit")
clear
if [[ $opt1 == "exit" ]]
then
    break
...
EOF
        echo "Press :'Enter' to confirm , 'Esc' to return back"
        echo
        opt2=$(gum file ~/.config/hypr/brain/config/animations/)
        if [[ "$opt2" ]]
        then
