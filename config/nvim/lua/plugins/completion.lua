return {
  ...
  opts = function()
    ...
    mapping = cmp.mapping.preset.insert({
      ...
      ["<S-Tab>"] = LazyVim.cmp.confirm({ behavior = cmp.ConfirmBehavior.Replace }), -- Accept currently selected item. Set `select` to `false` to only confirm explicitly selected items.
